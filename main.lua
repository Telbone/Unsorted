...
SMODS.Joker{
	...
	loc_txt = {
		...
		text = {
			'Gains {X:mult,C:white}X#2#{} every time a',
...
SMODS.Joker{
	...
	calculate = function(self,card,context)
	 if context.setting_blind and not context.blueprint and G.GAME.dollars >= 20 then
	   card.ability.extra.mult = card.ability.extra.mult + card.ability.extra.mult_mod
...
SMODS.Joker{
	...
	calculate = function(self,card,context)
	  ...
	  elseif context.end_of_round and not context.blueprint and G.GAME.blind.boss and card.ability.extra.mult > 0 then
...
SMODS.Joker{
    ...
    calculate = function(self,card,context)
	  ...
	  elseif context.after and not context.blueprint and card.ability.extra.Xmult > 1 and not context.blueprint then
...
SMODS.Joker{
    ...
    calculate = function(self,card,context)
	  ...
	  if context.end_of_round and not context.blueprint and card.ability.extra.chips > 0 then
	...
	update = function(self,card,dt)
	 if G.STAGE == G.STAGES.RUN then
	  local jokers = {
		 ...
		 'The Reaper', 'Short Temper', 'Miner', 'The Pyramid', 'Starry Night', 'Moonlight', 'Sunrise', 'Judge', 'Soil',
	  ...
	  if (G.GAME.dollars >= 146 and G.GAME.dollars <= 150) or G.GAME.dollars > 188 or G.GAME.dollars < 1 then
	   card.ability.extra.joker = 'None'
      else
	      card.ability.extra.joker = jokers[G.GAME.dollars]
	  end
	 end
	end,
    calculate = function(self,card,context)
	 ...
	 if context.selling_self and not context.blueprint and (G.GAME.dollars <= 146 or G.GAME.dollars >= 150) and G.GAME.dollars <= 188 and G.GAME.dollars >= 1 then
	    local card = create_card('Joker', G.jokers, nil, nil, nil, nil, slugs[G.GAME.dollars])
        card:add_to_deck()
...
SMODS.Joker{
	...
	loc_txt = {
		...
		text = {
			...
			'{C:inactive,s:0.8}Destroy extra Joker when{} {C:legendary,s:0.8}Essencia{} {C:inactive,s:0.8}is lost{}'
		}
	},
	atlas = 'atlas1',
	pos = {x = 10, y = 3},
...
function Card:remove_from_deck(from_debuff)
   if self.added_to_deck then
      if self.ability.name == 'Essencia' then
	     for i=1, #G.jokers.cards do
		    if G.jokers.cards[i].config.center.rarity == 4 and G.jokers.cards[i].ability.eternal then
			   G.jokers.cards[i].ability.eternal = nil
			   G.jokers.cards[i]:start_dissolve(nil, true)
